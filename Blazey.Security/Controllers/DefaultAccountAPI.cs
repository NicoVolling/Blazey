...
[Controller]
public class DefaultAccountAPI<T_User> : AccountAPIController<T_User> where T_User : IdentityUser<Guid>, IUser, new()
{
    ...
    public DefaultAccountAPI(SignInManager<T_User> signInManager, UserManager<T_User> userManager, AdminCodeService adminCodeService, EmailService emailService) : base(userManager)
    {
    ...
    public async Task<IActionResult> RegisterAdmin(string AdminCode, string Username, string Password, string ReturnUrl)
    {
        ...
        {
            T_User User = new() { UserName = Username, Email = Username + "@mail.com", EmailConfirmed = true };
    ...
    [HttpPost("[action]")]
    public async Task<IActionResult> RegisterAsync(string Email, string Password, string ReturnUrl)
    {
        T_User User = new T_User() { UserName = Email, Email = Email };
        return await registerAsync(User, Password, ReturnUrl);
    }

    protected async Task<IActionResult> registerAsync(T_User User, string Password, string ReturnUrl)
    {
        if (await DoRegisterAsync(User, Password) is IdentityResult result)
        {
            if (result.Succeeded)
            {
                if (await userManager.FindByNameAsync(User.UserName!) is T_User user)
                {
                    string token = Convert.ToBase64String(Encoding.ASCII.GetBytes(await userManager.GenerateEmailConfirmationTokenAsync(user)));
                    try
                    {
                        await sendConfirmationEmail(user, $"{Request.Scheme}://{Request.Host.Value}/account/confirm/{user.Id.ToString("N")}/{token}");
    ...
    protected virtual async Task sendConfirmationEmail(T_User User, string link)
    {
        await emailService.SendEmailAsync(User.Email ?? string.Empty, "Email-Adresse für GameIT bestätigen", $"Hallo,<br><br>Klicken Sie auf den folgenden Link, um Ihre Email-Adresse zu bestätigen:<br><br>{$"<a href=\"{link}\">Hier klicken um Email zu Bestätigen</a>"}<br><br>Vielen Dank!<br><br>Das GameIT-Team");
    }
